...
export default function PersetujuanSPJBendahara() {
  ...
  const stats = {
    belum_spj: allData.filter((d) => d.status === "belum_spj")
      .length,
    menunggu_verifikasi_bendahara: allData.filter(
      (d) => d.status === "menunggu_verifikasi_bendahara",
    ).length,
    ...
    const updateLocal = (prevData: LaporanData[]) =>
      prevData.map((item) =>
        item.noSppd === selectedLaporan.noSppd
          ? { ...item, status: "menunggu_verifikasi_bendahara" as const }
          : item,
      );
  ...
  const handleConfirmBayar = (file: File) => {
    ...
    logActivity(
      ...
      `Dibayar oleh ${user?.nama || "Bendahara"}`,
      item.noSppd
    );
    ...
    // Auto redirect to Arsip SPJ after a short delay
    setTimeout(() => {
      navigate('/bendahara/arsip-spj');
    }, 1500);
  };
  ...
  return (
    <div className="min-h-screen bg-[#f7f9fb]">
          ...
          <div className="bg-[#f2f4f6] p-6 rounded-2xl flex flex-col lg:flex-row gap-6 items-center">
            ...
            <div className="w-full lg:flex-1 grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <button
                onClick={() =>
                  setStatusFilter(
                    statusFilter === "menunggu_verifikasi_bendahara"
                      ? "all"
                      : "menunggu_verifikasi_bendahara",
                  )
                }
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-lg font-bold text-[#5f3800]">
                      {String(
                        stats.menunggu_verifikasi_bendahara,
                      ).padStart(2, "0")}{" "}
                      Berkas
                    </p>
                  </div>
      ...
      <VerifikasiDokumenDialog
        ...
        footerActions={
          selectedLaporanToReview?.status === "menunggu_verifikasi_bendahara" ? (
            <>
              <button
              ...
              <button
                onClick={async () => {
                  ...
                  logActivity(
                    'status_spj',
                    `SPJ ${selectedLaporanToReview.noSppd} Disetujui Bendahara`,
                    `Disetujui oleh Bendahara (${user?.nama || "Bendahara"})`,
                    selectedLaporanToReview.noSppd
                  );

                  const updateLocal = (prevData: LaporanData[]) => prevData.map(d => d.noSppd === selectedLaporanToReview.noSppd ? { ...d, status: "menunggu_verifikasi_pptk" as const } : d);
                  if (selectedLaporanToReview.tipePerjalanan === "Dalam Daerah") setDalamDaerahData(updateLocal);
                  else setLuarDaerahData(updateLocal);
                  
                  try {
                    localStorage.setItem(`status_laporan_${selectedLaporanToReview.noSppd}`, "menunggu_verifikasi_pptk");
                  } catch(e) {}
                  ...
                  if ((selectedLaporanToReview as any).id) {
                    apiRequest(`/laporan/${(selectedLaporanToReview as any).id}`, {
                      method: 'POST',
                      body: JSON.stringify({ status: "menunggu_verifikasi_pptk" }),
                    }).catch(console.error);
                  }
            ...
            <div className="p-5 border-t border-slate-200 bg-slate-50 flex justify-end gap-3">
              ...
              <button
                onClick={() => {
                  ...
                  logActivity(
                    'status_spj',
                    `SPJ ${selectedLaporanToReview.noSppd} Direvisi (Bendahara)`,
                    `Revisi oleh Bendahara (${user?.nama || "Bendahara"}): ${revisiNote || 'Tidak ada catatan'}`,
                    selectedLaporanToReview.noSppd
                  );
