...
export default function ArsipSPJKPA() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setDalamDaerahData(combinedDalam.filter(isApproved).map(hydrateData).filter(d => d.status !== 'belum_spj' && d.status !== 'draft_laporan'));
      setLuarDaerahData(combinedLuar.filter(isApproved).map(hydrateData).filter(d => d.status !== 'belum_spj' && d.status !== 'draft_laporan'));
    } catch (err) {
  ...
  useEffect(() => {
    const redirectSppd = localStorage.getItem('redirect_spj_sppd');
    const redirectDokumen = localStorage.getItem('redirect_arsip_dokumen');

    if ((redirectSppd || redirectDokumen) && !isLoadingData && [...dalamDaerahData, ...luarDaerahData].length > 0) {
      const allItems = [...dalamDaerahData, ...luarDaerahData];
      const targetSppd = redirectSppd || redirectDokumen;
      const item = allItems.find(d => (d.noSppd || d.no_sppd || '') === targetSppd);
      if (item) {
        setSelectedLaporan(item);
        if (redirectDokumen) {
          setIsVerifikasiDialogOpen(true);
        } else {
          if (item.tipePerjalanan === 'Dalam Daerah') {
            setIsDialogOpen(true);
          } else {
            setIsLuarDaerahDialogOpen(true);
          }
        }
        setSearchQuery(targetSppd!);
        setStatusFilter('all');
      }
      localStorage.removeItem('redirect_spj_sppd');
      localStorage.removeItem('redirect_arsip_dokumen');
    }
  }, [isLoadingData, dalamDaerahData, luarDaerahData]);
  ...
  const stats = {
    belum_spj: allData.filter((d) => d.status === "belum_spj")
      .length,
    menunggu_verifikasi_kpa: allData.filter(
      (d) => d.status === "menunggu_verifikasi_kpa",
    ).length,
    menunggu_pembayaran: allData.filter(
      (d) => d.status === "menunggu_pembayaran",
    ).length,
        ...
        <div className="space-y-10">
          ...
          <div className="bg-[#f2f4f6] p-6 rounded-2xl flex flex-col lg:flex-row gap-6 items-center">
            ...
            <div className="hidden lg:block h-12 w-px bg-slate-300"></div>
              ...
              {/* Intentionally removed Status Cards for Arsip */}
          </div>
