...
import { BayarDialog } from "../components/BayarDialog";
...
export default function PersetujuanSPJPegawai() {
  ...
  const [selectedLaporanToBayar, setSelectedLaporanToBayar] = useState<LaporanData | null>(null);
  ...
  const filteredData = allData.filter((item) => {
    // Pegawai should only see SPJs where their NIP is in pelaksana
    const userNip = (user?.nip || "").replace(/\D/g, "");
    const isPelaksana = Array.isArray(item.pelaksana) && item.pelaksana.some((p: any) => (p.nip || "").replace(/\D/g, "") === userNip);
    if (!isPelaksana && user?.role !== 'admin') return false;
  ...
  const stats = {
    belum_spj: allData.filter((d) => d.status === "belum_spj")
      .length,
    menunggu_verifikasi_pegawai: allData.filter((d) => {
    ...
    const exportData = filteredData.map((item) => ({
      ...
      Status:
        item.status === "belum_spj"
          ? "Belum SPJ"
          : item.status === "menunggu_verifikasi_pegawai"
            ? "Menunggu Verifikasi Pegawai"
    ...
    const updateLocal = (prevData: LaporanData[]) =>
      prevData.map((item) =>
        item.noSppd === selectedLaporan.noSppd
          ? { ...item, status: "menunggu_verifikasi_pegawai" as const }
          : item,
      );
  ...
  const handleConfirmBayar = async (file: File) => {
    ...
    toast.success("SPJ Berhasil Dibayar Lunas!");
    
    // Catat log aktivitas
    logActivity(
      'status_spj',
      `SPJ ${item.noSppd} Dibayar Lunas`,
      `Dibayar oleh ${user?.nama || "Pegawai"}`,
      item.noSppd
    );

    const updateLocal = (prevData: LaporanData[]) => prevData.map(d => d.noSppd === item.noSppd ? { ...d, status: "selesai" as const } : d);
    if (tipePerjalanan === "Dalam Daerah") setDalamDaerahData(updateLocal);
    ...
    if ((item as any).id) {
      apiRequest(`/laporan/${(item as any).id}`, {
        method: 'POST',
        ...
        if (skData) {
          ...
          const updated = parsed.map((sk: any) => {
            if (sk.nama === subKegiatanName) {
              return {
                ...sk,
                realisasi: sk.realisasi + item.totalAnggaran,
                sisa: sk.sisa - item.totalAnggaran
              };
            }
            return sk;
          });
    ...
    setSelectedLaporanToBayar(null);
  };
  ...
  const getStatusBadge = (item: LaporanData) => {
    switch (item.status) {
      case "belum_spj":
        return (
      ...
      case "menunggu_verifikasi_pegawai": {
        ...
        return (
          <span className="px-3 py-1 bg-amber-50 text-amber-700 text-xs font-bold rounded-full border border-amber-200">
            {hasApproved ? `Menunggu Rekan (${approvals.length}/${item.pelaksana.length})` : "Menunggu Anda"}
          </span>
        );
      }
  ...
  return (
    <div className="min-h-screen bg-[#f7f9fb]">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div className="max-w-2xl">
              <h3 className="text-3xl md:text-4xl font-bold text-[#00475e] tracking-tight">
                Persetujuan SPJ - Pegawai
              </h3>
              <p className="text-[#40484d] mt-3 text-lg leading-relaxed">
                Review dan setujui laporan pertanggungjawaban (SPJ) perjalanan dinas Anda.
              </p>
            </div>
          ...
          <div className="bg-[#f2f4f6] p-6 rounded-2xl flex flex-col lg:flex-row gap-6 items-center">
            ...
            <div className="w-full lg:flex-1 grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <button
                onClick={() =>
                  setStatusFilter(
                    statusFilter === "menunggu_verifikasi_pegawai"
                      ? "all"
                      : "menunggu_verifikasi_pegawai",
                  )
                }
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-lg font-bold text-[#5f3800]">
                      {String(
                        stats.menunggu_verifikasi_pegawai,
                      ).padStart(2, "0")}{" "}
                      Berkas
                    </p>
                  </div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-lg font-bold text-emerald-800">
                      {String(stats.menunggu_pembayaran).padStart(2, "0")} Berkas
                    </p>
                  </div>
                </div>
              </button>
                        ...
                        <td className="px-6 py-5 text-center">
                          {item.status === "menunggu_verifikasi_pegawai" ? (
                            ...
                            <div className="flex justify-center gap-2">
                              <button
                                onClick={() => {
                                  setSelectedLaporanToBayar(item);
                                  setIsBayarDialogOpen(true);
                                }}
                                className="px-3 py-1.5 bg-emerald-600 text-white rounded-lg text-xs font-bold hover:bg-emerald-700 shadow-sm transition-all active:scale-95 flex items-center gap-1"
                              >
      ...
      {/* Review Dialog */}
      {isReviewDialogOpen && selectedLaporanToReview && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-slate-200 flex justify-between items-center bg-slate-50">
              <div>
                <h3 className="text-xl font-bold text-[#00475e]">Periksa Laporan SPJ</h3>
                <p className="text-sm text-slate-500 mt-1">No. SPT: {selectedLaporanToReview.noSpt}</p>
              </div>
              <button
                onClick={() => {
            ...
            <div className="p-6 overflow-y-auto flex-1">
              <h4 className="font-semibold text-slate-700 mb-4">Daftar Kelengkapan Dokumen</h4>
              <div className="space-y-3">
                {[
                  ...(selectedLaporanToReview.pelaksana.findIndex((p: any) => (p.nip || "").replace(/[\s-]/g, "") === (user?.nip || "").replace(/[\s-]/g, "")) === 0 
                    ? [{ id: 'kwitansi', name: 'Kwitansi Dinas' }] 
                    : []),
                  { id: 'rincian', name: 'Rincian Perjalanan' },
                ].map((doc) => (
                  <div key={doc.id} className="flex items-center justify-between p-4 bg-slate-50 border border-slate-200 rounded-xl">
                    ...
                    <button 
                      ...
                      className="px-3 py-1.5 bg-[#e6f3f8] text-[#00475e] hover:bg-[#cde7f0] rounded-lg text-xs font-bold transition-colors flex items-center gap-1"
                    >
                ...
                {!(user?.nip && (pegawaiApprovalsMap[selectedLaporanToReview.noSppd] || []).includes(user.nip)) ? (
                  <>
                    <button
                      ...
                      className="px-4 py-2 bg-amber-500 text-white rounded-xl text-sm font-bold hover:bg-amber-600 shadow-sm transition-all active:scale-95 flex items-center gap-2"
                    >
                ...
                <button
                  onClick={async () => {
                    ...
                    if (approvals.length >= pelaksanaCount) {
                      toast.success("Semua pegawai telah menyetujui! Diteruskan ke Bendahara.", { id: toastId });
                      
                      // Catat log aktivitas
                      logActivity(
                        'status_spj',
                        `SPJ ${selectedLaporanToReview.noSppd} Disetujui Pegawai`,
                        `Disetujui oleh Pegawai (${user?.nama || "Pegawai"})`,
                        selectedLaporanToReview.noSppd
                      );

                      const updateLocal = (prevData: LaporanData[]) => prevData.map(d => d.noSppd === selectedLaporanToReview.noSppd ? { ...d, status: "menunggu_verifikasi_bendahara" as const } : d);
                      ...
                      if ((selectedLaporanToReview as any).id) {
                        apiRequest(`/laporan/${(selectedLaporanToReview as any).id}`, {
                          method: 'POST',
                          body: JSON.stringify({ status: "menunggu_verifikasi_bendahara" }),
                        }).catch(console.error);
                      }
                    } else {
                      toast.success(`Disetujui! Menunggu persetujuan rekan lainnya (${approvals.length}/${pelaksanaCount})`, { id: toastId });
                      
                      // For UI refresh (removing it from pending if the current user approved it)
                      // Actually, if we want to remove it from the list for THIS user, we need to ensure the list filters out approved ones.
                      // Let's trigger a re-render.
                      if (selectedLaporanToReview.tipePerjalanan === "Dalam Daerah") {
                         setDalamDaerahData(prev => [...prev]);
                      } else {
                         setLuarDaerahData(prev => [...prev]);
                      }
                    }
                  ...
                  className="px-4 py-2 bg-[#00475e] text-white rounded-xl text-sm font-bold hover:bg-[#1a5f7a] shadow-sm transition-all active:scale-95 flex items-center gap-2"
                >
            ...
            <div className="p-5 border-t border-slate-200 bg-slate-50 flex justify-end gap-3">
              ...
              <button
                onClick={async () => {
                  ...
                  logActivity(
                    'status_spj',
                    `SPJ ${selectedLaporanToReview.noSppd} Direvisi (Pegawai)`,
                    `Revisi oleh Pegawai (${user?.nama || "Pegawai"}): ${revisiNote || 'Tidak ada catatan'}`,
                    selectedLaporanToReview.noSppd
                  );
      ...
      {/* Bayar Dialog */}
      <BayarDialog
        isOpen={isBayarDialogOpen}
        onClose={() => {
          setIsBayarDialogOpen(false);
          setSelectedLaporanToBayar(null);
        }}
        onConfirm={handleConfirmBayar}
        data={selectedLaporanToBayar}
      />
