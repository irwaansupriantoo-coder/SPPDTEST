...
export default function PersetujuanSPJKPA() {
  const { user } = useAuth();
  ...
  const stats = {
    belum_spj: filteredData.filter((d) => d.status === "belum_spj")
      .length,
    menunggu_verifikasi_kpa: filteredData.filter(
  ...
  return (
    <div className="min-h-screen bg-[#f7f9fb]">
          ...
          <div className="bg-[#f2f4f6] p-6 rounded-2xl flex flex-col lg:flex-row gap-6 items-center">
            ...
            <div className="w-full lg:flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <button
                onClick={() =>
                  setStatusFilter(
                    statusFilter === "menunggu_verifikasi_kpa"
                      ? "all"
                      : "menunggu_verifikasi_kpa",
                  )
                }
      ...
      <VerifikasiDokumenDialog
        ...
        footerActions={
          selectedLaporanToReview?.status === "menunggu_verifikasi_kpa" ? (
            <>
              <button
              ...
              <button
                onClick={async () => {
                  ...
                  logActivity(
                    'status_spj',
                    `SPJ ${selectedLaporanToReview.noSppd} Disetujui KPA`,
                    `Disetujui oleh KPA (${user?.nama || "KPA"})`,
                    selectedLaporanToReview.noSppd
                  );

                  const updateLocal = (prevData: LaporanData[]) => prevData.map(d => d.noSppd === selectedLaporanToReview.noSppd ? { ...d, status: "menunggu_pembayaran" as const } : d);
                  ...
                  if ((selectedLaporanToReview as any).id) {
                    apiRequest(`/laporan/${(selectedLaporanToReview as any).id}`, {
                      method: 'POST',
                      body: JSON.stringify({ status: "menunggu_pembayaran" }),
                    }).catch(console.error);
                  }
            ...
            <div className="p-5 border-t border-slate-200 bg-slate-50 flex justify-end gap-3">
              ...
              <button
                onClick={async () => {
                  ...
                  logActivity(
                    'status_spj',
                    `SPJ ${selectedLaporanToReview.noSppd} Direvisi (KPA)`,
                    `Revisi oleh KPA (${user?.nama || "KPA"}): ${revisiNote || 'Tidak ada catatan'}`,
                    selectedLaporanToReview.noSppd
                  );
