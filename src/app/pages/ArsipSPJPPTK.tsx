...
export default function ArsipSPJPPTK() {
  ...
  useEffect(() => {
    ...
    if ((redirectSppd || redirectDokumen) && !isLoadingData && [...dalamDaerahData, ...luarDaerahData].length > 0) {
      ...
      const item = allItems.find(d => (d.noSppd || (d as any).no_sppd || '') === targetSppd);
      if (item) {
        setSelectedLaporan(item as LaporanData);
        if (redirectDokumen) {
        ...
        setSearchQuery(targetSppd as string);
        setStatusFilter('all');
      }
  ...
  const filteredData = allData.filter((item) => {
    ...
    const matchesSearch =
      searchQuery === "" ||
      (item.noSppd || (item as any).no_sppd || "")
        .toLowerCase()
