...
export default function ArsipSPJPengelola() {
  const { user } = useAuth();
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setDalamDaerahData(combinedDalam.filter(isApproved).map(hydrateData));
      setLuarDaerahData(combinedLuar.filter(isApproved).map(hydrateData));
    } catch (err) {
