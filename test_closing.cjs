...
async function run() {
  ...
  cells.forEach(c => {
    const fullMatch = sheet1.match(new RegExp('<c r="' + c + '"[^>]*>[\\s\\S]*?</c>'));
    const selfClosingMatch = sheet1.match(new RegExp('<c r="' + c + '"[^>]*/>'));
    console.log(c + ':', fullMatch ? 'FULL' : (selfClosingMatch ? 'SELF_CLOSING' : 'NONE'));
  });
}
run();
